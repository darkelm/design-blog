/**
 * Ghost CMS Provider Implementation
 * 
 * Implementation of the CMSProvider interface for Ghost CMS.
 * Encapsulates all Ghost-specific API calls and logic.
 */
 ...
 * Ghost CMS Provider
 * 
 * Implements the CMSProvider interface using Ghost Content API.
 */
export class GhostCMSProvider implements CMSProvider {
  ...
  async getPostsByTag(slug: string, limit = 10): Promise<Post[]> {
    try {
      const response = await this.api.posts.browse({
        limit,
        filter: `tag:${slug}`,
  ...
  async getPostsByAuthor(slug: string, limit = 10): Promise<Post[]> {
    try {
      const response = await this.api.posts.browse({
        limit,
        filter: `author:${slug}`,
  ...
  async getTagBySlug(slug: string): Promise<Tag> {
    try {
      const tag = await this.api.tags.read({ slug })
      if (!isTag(tag)) {
        throw new NotFoundError(`Tag: ${slug}`)
      }
      return tag
  ...
  async getAuthors(): Promise<Author[]> {
    try {
      const response = await this.api.authors.browse({
        limit: 'all',
        include: ['count.posts'],
      })
  ...
  async getAuthorBySlug(slug: string): Promise<Author> {
    try {
      const author = await this.api.authors.read(
        { slug },
        { include: ['count.posts'] }
      )
      if (!isAuthor(author)) {
  ...
  async getPages(): Promise<Page[]> {
    try {
      const response = await this.api.pages.browse({ limit: 'all' })
