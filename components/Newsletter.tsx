...
export function Newsletter() {
  ...
  return (
    <section className="bg-neutral-100">
      <div className="mx-auto max-w-content px-fluid py-20 lg:py-24">
        <motion.div
          ...
          {status === 'success' ? (
            ...
            <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
              <input
                ...
                required
                className="flex-1 px-4 py-3 bg-white border border-neutral-200 rounded-lg text-body-md placeholder:text-neutral-400 focus:outline-none focus:ring-2 focus:ring-neutral-900 focus:border-transparent transition-shadow"
              />
              <button
                type="submit"
                disabled={status === 'loading'}
                className="px-6 py-3 bg-neutral-900 text-white font-medium rounded-lg hover:bg-neutral-800 disabled:opacity-60 disabled:cursor-not-allowed transition-colors"
              >
                {status === 'loading' ? (
